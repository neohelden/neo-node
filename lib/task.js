...
function initializeNats() {
  ...
  nats.on('connect', (nc) => logger.info(`[NPQ] Connected to ${nc.currentServer.url.host}`))
  nats.on('disconnect', () => logger.info('[NPQ] Disconnect'))
  nats.on('reconnecting', () => logger.info('[NPQ] Reconnecting'))
  nats.on('reconnect', (nc) => logger.info(`[NPQ] Reconnect ${nc.currentServer.url.host}`))
  nats.on('close', () => logger.debug('[NPQ] Close'))
}
    ...
    return new Promise((resolve, reject) => {
      nats.requestOne(queue, particle, {}, _.get(opts, 'timeout', TASK_TIMEOUT), (response) => {
        if (response instanceof Nats.NatsError && response.code === Nats.REQ_TIMEOUT) {
          ...
          let err = new Error(_.get(response, 'error.message', 'Unknown error'))
  ...
  process: (queue, processor) => { // pun intended
    nats.subscribe(queue, async function (particle, replyTo, subject) {
      ...
      try {
        ...
        logger.error(`[NEO:TASK] Error processing task for ${queue} (Sentry-ID: ${sentryId})`)
